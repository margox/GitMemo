...
**Claude Doc Sync 是一个面向开发者的 AI 知识管理工具，它同时做两件事：**
...
> Claude Doc Sync 自动记录你与 AI 的每一次对话，同时支持你随时创建笔记，所有内容通过 Git 同步，数据完全属于你。

### 与 Novi Notes 的定位差异

| 维度 | Novi Notes | Claude Doc Sync |
|------|-----------|----------------|
...
- Claude Doc Sync 是一个**AI 对话备份工具**，同时支持笔记
...
**这意味着：`claude-doc-sync init` 只需要注入配置，不需要启动任何后台进程。**
...
**保存位置**：`~/.claude-doc-sync/conversations/{YYYY-MM}/{MM-DD}-{标题摘要}.md`
...
```json
{
  ...
  "hooks": [{
    "type": "command",
    "async": true,
    "command": "FILE=$(cat /dev/stdin | python3 -c \"import sys,json; d=json.load(sys.stdin); print(d.get('tool_input',{}).get('file_path',''))\" 2>/dev/null); SYNC_DIR=\"$HOME/.claude-doc-sync\"; if echo \"$FILE\" | grep -q \"^$SYNC_DIR/\"; then cd \"$SYNC_DIR\" && git add -A && git commit -m \"auto: save $(basename \"$FILE\")\" && git push origin main 2>/dev/null; fi"
  }]
}
```
...
│  • 运行 claude-doc-sync help 查看更多命令         │
└─────────────────────────────────────────────────┘
```
...
```bash
# 快速记一条便签
claude-doc-sync note "今天发现 Rust 的 async trait 终于稳定了"

# 写今天的日记（打开编辑器）
claude-doc-sync daily

# 创建一篇手册
claude-doc-sync manual "Rust 异步编程指南"
...
```bash
# 搜索所有对话和笔记
claude-doc-sync search "rust async"
...
# 查看最近的对话
claude-doc-sync recent

# 查看统计
claude-doc-sync stats
```
...
```
  ...
  云端存储  │  Notion    │                     │          │
            │  Evernote  │                     │          │
            ├───────────┼─────────────────────┼──────────┤
  Git 同步  │            │            Claude Doc Sync     │
            │            │           ← 唯一占位 →         │
            └───────────┼─────────────────────┼──────────┘
                        │                     │
```

**Claude Doc Sync 独占的象限：Git 同步 + 自动记录 + 主动笔记**

### 逐一对比

| 维度 | Novi Notes | Obsidian | Notion | Claude Doc Sync |
|------|-----------|----------|--------|----------------|
| 对话自动记录 | ❌ | ❌ | ❌ | ✅ |
| 笔记功能 | ✅ 丰富 | ✅ 强大 | ✅ 全能 | ✅ 轻量 |
| Git 版本控制 | ❌ | ⚠️ 插件 | ❌ | ✅ 原生 |
...
所有数据都是 Markdown 文件。不依赖私有格式。即使用户卸载了 Claude Doc Sync，数据依然可以用任何文本编辑器打开。
...
[了解]
 ...
 ↓
[安装]  ← 关键节点：转化率目标 > 80%
 运行 brew install claude-doc-sync 或 curl 安装
 耗时 < 30 秒
 ↓
[初始化]  ← 关键节点：完成率目标 > 90%
 运行 claude-doc-sync init
...
```toml
# ~/.claude-doc-sync/.sync-config.toml
